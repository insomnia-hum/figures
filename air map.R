...
city = read_sf('C:/pHD刘泽源/区划/市.shp')
library(readxl)
for (i in 1:8) {
pred = predict(model,box[[i]])%>%as.numeric()
ms = read_xlsx('C:/pHD刘泽源/刘泽源/manuscript2/2018版经纬度.xlsx')
index = read_xlsx('C:/pHD刘泽源/刘泽源/manuscript2/coeff2.xlsx')
...
ninedashline = read_sf('C:/pHD刘泽源/区划/海岸线.shp')

city2 = city%>%gather(key = 'scenario',value = 'pm',7:14)
grid = read_sf('C:/pHD刘泽源/刘泽源/manuscript2/grid/grid.shp')
ggplot(grid)+geom_sf(data = city2,aes(fill = pm))+scale_fill_viridis_b(direction = -1)+geom_sf(data = ninedashline)+facet_wrap(~factor(scenario,levels = c('Baseline','CCUS0','CCUS1','CCUS2','CCUS2-NET1','CCUS2-NET2','CCUS2-NET2-MFRT','CCUS0-MFRT')),
                                                                                                                              nrow = 2,ncol = 4)+
...
ggsave('./airmap.png',height = 8,width = 10,dpi = 600)
